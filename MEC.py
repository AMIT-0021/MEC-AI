...
# Custom CSS for dark-theme enhancement and styled cards
st.markdown("""
    <style>
    .main {
...
col1.metric("Model R² Score", f"{r2:.2f}", delta="Optimal Fit", delta_color="normal")
col2.metric("Mean Absolute Error", f"{mae:.2f} mL", delta="-1.2% Error Margin", delta_color="inverse")
col3.metric("Dataset Records", f"{len(df)} Sensor Logs", delta="Active Connection")
...
st.sidebar.markdown("Adjust active parameters to simulate hydrogen output:")
...
fig = px.scatter(
    ...
    y="Predicted H2 (mL)",
    trendline="ols",
    title="Real-time Model Validation Scatter Plot",
...
fig.update_layout(
    xaxis_title="Actual H₂ Production (mL)",
    yaxis_title="Predicted H₂ Production (mL)",
...
with st.expander("📑 Explore Full Training Dataset"):
